...
type FormValues = z.infer<
  typeof form1023Part2OrganizationalStructureStep4Schema
>;

export default function FormationPart2Step4Page() {
  const router = useRouter();

  const [isLoading, setIsLoading] = useState<boolean>(false);

  const form = useForm<FormValues>({
    resolver: zodResolver(form1023Part2OrganizationalStructureStep4Schema),
  });
  ...
  async function onSubmit(values: FormValues) {
    ...
    try {
      ...
      if (response?.status !== 200) {
        throw new Error(
          `Formation part 2 step 4 response status: ${response?.status}`,
        );
      }

      router.push("/formation/part-2/step-5");
    } catch (err) {
      // TODO
        ...
        <Form {...form}>
          ...
          >
            ...
            <FormationNavigationButtons
              prevHref="/formation/part-2/step-3"
              isLoading={isLoading}
            />
          </form>
        </Form>
