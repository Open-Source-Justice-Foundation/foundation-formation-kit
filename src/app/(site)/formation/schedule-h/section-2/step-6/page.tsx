  ...
  async function onSubmit(values: FormValues) {
    ...
    try {
      ...
      if (response?.status !== 200) {
        throw new Error(
          `Formation schedule H section 2 step 6 response status: ${response?.status}`,
        );
      }

      router.push("/formation/schedule-h/section-2/step-7");
    } catch (err) {
      // TODO
    ...
    <Card className="flex w-[360px] flex-col border max-[444px]:mx-6 max-[444px]:w-[88%] sm:w-[425px] md:border-0">
      <CardHeader className="px-4 pt-4 sm:px-6 sm:pt-6">
        <CardTitle className="text-base sm:text-xl md:text-2xl">
          Educational Support for Employees of a Particular Employer
        </CardTitle>
      </CardHeader>
          ...
          >
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  <FormDescription>
                    Do you or will you provide scholarships, fellowships, or
                    educational loans to attend an educational institution to
            ...
            <FormationNavigationButtons
              prevHref="/formation/schedule-h/section-2/step-5"
              isLoading={isLoading}
            />
          </form>
        </Form>
