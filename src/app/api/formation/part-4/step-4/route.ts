...
export async function POST(request: NextRequest): Promise<NextResponse> {
  ...
  try {
    ...
    if (err instanceof ZodError) {
      throw new Error(
        "Failed to process formation part 4 step 4: invalid data",
      );
    }
    console.error(err);
    throw new Error("Failed to process formation part 4 step 4");
  }

  return NextResponse.json(
    { message: "Formation part 4 step 4 submitted" },
    { status: 200 },
  );
}
